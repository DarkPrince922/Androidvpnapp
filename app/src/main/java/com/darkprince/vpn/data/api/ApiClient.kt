...
class ApiClient(private val prefs: AppPrefs) {
    ...
    suspend fun refreshTokens(force: Boolean = false): String? = refreshMutex.withLock {
        ...
        try {
            ...
            // Выкидываем из аккаунта только на 401. Раньше сюда попадал любой
            // ответ 4xx, и человек оказывался на экране входа из-за 429
            // «слишком часто» или 404 от страницы провайдера вместо кабинета —
            // при живой, ничем не испорченной сессии, и обратной дороги не
            // было: refresh-токен уже стёрт.
            //
            // 401 достаточно: /cabinet/auth/refresh отвечает им на любую беду
            // с токеном — просрочен, отозван, нет в базе, аккаунт не активен, —
            // и никогда не отвечает 403. Значит 403 пришёл не от кабинета, а
            // от чего-то по дороге, и трогать из-за него сессию нельзя.
            if (e.code() == 401) {
                val detail = runCatching { e.response()?.errorBody()?.string() }.getOrNull()
                prefs.noteSessionEnded(e.code(), detail)
                com.darkprince.vpn.data.log.AppLog.write("сессия завершена сервером: 401 $detail")
                prefs.setTokens(null, null, null)
            } else {
                com.darkprince.vpn.data.log.AppLog.write("обновление сессии не удалось: ${e.code()}")
            }
            null
        } catch (_: Exception) {
