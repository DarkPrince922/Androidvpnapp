...
fun Exception.userMessage(): String = when (this) {
    is retrofit2.HttpException -> when (code()) {
        ...
        // при покупке, продлении, пакете трафика, докупке устройств. Своего
        // разбора у нас не было, и человек видел «Ошибка сервера (402)»:
        // слова, из которых ни причина, ни выход не следуют.
        402 -> insufficientFundsMessage()
        403 -> "Доступ запрещён."
...
/**
 * Сколько не хватило, если сервер это сказал.
 *
 * Тело ошибки у кабинета разной формы: где-то объект с суммами, где-то
 * английская строка. Вытаскиваем только число копеек — на нём одном
 * держится вся польза сообщения, а разбирать остальное ради текста,
 * который всё равно показывать нельзя, незачем.
 */
private fun retrofit2.HttpException.insufficientFundsMessage(): String {
    val body = runCatching { response()?.errorBody()?.string() }.getOrNull()
    val missing = body
        ?.let { Regex(""""missing_kopeks"\s*:\s*(\d+)""").find(it) }
        ?.groupValues?.get(1)
        ?.toLongOrNull()
    return if (missing != null && missing > 0) {
        val rubles = String.format(java.util.Locale.getDefault(), "%.2f", missing / 100.0)
        "Не хватает $rubles ₽ на балансе. Пополните счёт и повторите."
    } else {
        "Не хватает средств на балансе. Пополните счёт и повторите."
    }
}
