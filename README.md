...
- **Совместный доступ**: владелец показывает QR-код подписки («Ещё →
  Поделиться подпиской»), второй человек сканирует его при входе и работает в
  гостевом режиме — VPN доступен, кабинет и оплата нет. Каждое устройство
  занимает своё место в лимите устройств тарифа.
